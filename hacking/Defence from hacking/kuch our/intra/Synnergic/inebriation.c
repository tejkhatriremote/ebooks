/*
   ...
   - GOT overwrites
     ...
     It can be rewritten to include elf.h and read the got without intervention
     of binutils tools.
   ...
   Dangers:
     ...
     This program may incur a heavy weight on social relations whenever friends
...
<shouts ranting=scrippie, coolvibe>
   Lamagra, zen-parse, JimJones, slash, #phreak.nl, dvorak, dethy, guidob,
   ...
   bezwaren), Escher (*splitting headache* *squint*), Fritsie Harmsen Ter Beek,
   Haagsche Harrie (nie te wehnag!), Steve Jobs, Jean-Louis Gassee, Clark Kent,
   ...
   *NO* Disney characters... THEY SUCK! *wankers*), Hex, Rincewind, Ridcully,
   ...
   Disclaimer:
      ...
      if damage to your software or other people their computer systems occurs,
...
char *createDString(unsigned long retaddy, unsigned int n_value)
{
   ...
   if(high < low) {     /* We swap low and higher adresses */
      bucket = low;     /* This is usefull on platforms with for instance */
      low = high;       /* Lower bound stacks or strange return addy's */
      high = bucket;    /* Ie. returning in libc/process space */
   }
