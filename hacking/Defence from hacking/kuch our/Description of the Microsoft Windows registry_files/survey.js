...
function closesurvey(id)
{
	...
	if (id == 0 || g_surveystyle=="embedded"){
		if (g_ISCONTACTUS)
			document.forms["frmSurveyMain"].action = "https://" + g_servername + "/contactus/feedback.aspx";
		else
			document.forms["frmSurveyMain"].action = "https://" + g_servername + "/common/survey.aspx";
		document.forms["frmSurveyMain"].submit();
		delayhalfsecond();
	}
	else if(typeof(g_ISCONTACTUS) == 'undefined' || !g_ISCONTACTUS)
	{
...
{	
	...
	if(currentsection!=null){	
		...
		if(previoussection!=null) previoussection.style.display = "none";//do not hide it when only last section
	}
	else	
		closesurvey(0);	
	navindex++;	
...
function addAnswer(sName, sOptionValue, sOptionText, sType, sAnswerID)
{
	...
	if (g_iAnswerPtr > 0 && (sType == "text" || sType == "textarea" || sType == "password"))
	{
		if (sQuestionID == g_asAnswers[g_iAnswerPtr-1][0] && sOptionID == g_asAnswers[g_iAnswerPtr-1][1] && '' == g_asAnswers[g_iAnswerPtr-1][3])
		{
			...
			g_asAnswers[g_iAnswerPtr-1][3] = urlEncode(sOptionText);						
			return;
		}
	}
	...
	g_asAnswers[g_iAnswerPtr][3] = urlEncode(sOptionText);
...
function urlEncode(str){
	...
	var perc = str.split("%");
	var encoded = escape(perc[0]);
	
	for(var i=1; i<perc.length; i++){
		encoded += "%25";
		encoded += escape(perc[i]);
	}
...
{
	//netscape 6 can nopt get the value from text area when div is display:none
	/*sections = document.getElementsByTagName("div");	
	for (index = 0; index < sections.length; index ++){
		if (sections[index].id.indexOf('DIV_') != -1)
		...
		{			
			case "radio":				
				if (e.checked)
				{
					...
					{
						var temptxt = e.title;//ContactUs will have text here, fms will not
			...
			case "select-one":				
				if (e.selectedIndex >= 0){
					...
					if (SectionModified[sname] == 1){ // if not modified then do not log.					
						...
						addAnswer(e.name, e.value, txt, sType, e.options[e.selectedIndex].id); // also log the text in select item for contactus
					}
				}
				break;
	...
	/*
	// can not show them back. with select-one question, netscape 6 will crash
	sections = document.getElementsByTagName("div");	
	for (index = 0; index < sections.length; index ++)
	{
		if (sections[index].id.indexOf('DIV_') != -1)
...
function writeAnswers(id, isKBEmbedded)
{
	...
	if (document.getElementById("CONTACTUSQUERYSTRINGS") != null)
		document.getElementById("CONTACTUSQUERYSTRINGS").value=g_CONTACTUSQUERYSTRINGS;	
	if (document.getElementById("CLIENTOSLANG") != null){
		if (typeof(window.navigator.systemLanguage) == 'undefined')
			document.getElementById("CLIENTOSLANG").value = window.navigator.language;
		else
			document.getElementById("CLIENTOSLANG").value = window.navigator.systemLanguage;		
	}
	...
	if (g_asParams != null){		
		...
		params.value = ''; // after submit and navigate back, this field can be non empty. So clean it first
...
{	
	if (typeof(event) == 'undefined')//mozilla
		return;
...
function findParent(el, type){
	...
	if(!p) return false;
	if(p.tagName && p.tagName.toLowerCase() == type) return p;
	else return findParent(p, type);
}
...
function handleOptionalTxt(el){
	...
	if(f){
		...
		for(var i=0; i<e.length-1; i++){
			if(e[i].type == "radio" && e[i+1].className == "OPTIONALTEXTBOX"){
				e[i+1].disabled = !e[i].checked;
			}
		}
	}
}
