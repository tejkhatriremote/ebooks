...
else if(window.removeEventListener){var MS_RemoveEvent = function(el,strEvent,funcPtr){el.removeEventListener(strEvent,funcPtr, false);}}
...
function MS_RPCHelper(id)
{
	...
	this.MakeRequest = function(url, onUnloadTimeout)
	{
...
function MS_PageToolsData()
{
	...
	this.GoToFavorites = function(el)
	{
		...
		el.href = document.location.protocol + '//' + document.location.hostname + "/personalization/mydisplaysettings.aspx";
		return true;
	}
...
function MS_StatusWindow()
{
	...
	this.Load = function()
	{
		...
		try
		{
			...
			this.RPCHelper.MakeRequest(this.iframeUrl, null);
			this.FadeIn();
...
function SwitchLanguage(containerId)
{
	...
	var ln = scid[1];
	var id = scid[2];	
	var redirecturl = document.location.protocol + '//' + document.location.hostname + '/' + type + '/' + id + '/' + ln;
	var redirectATag = document.getElementById("KBLangListRedirect");
	if (redirectATag != null)
	{
...
function MS_StatsDotNet()
{
	...
	{
		this.eventSeqNo = this.GetCookieIncrement();		//fix for bug 31271
		...
		document.StatsDotNetImg.src = ltsUrl;
	}
...
function MNPResizeFix()
{
	...
	var ss = MS_GetEl("SurveyStart");
	if(ss && !document.all) MS_AddEvent(ss, "click", netscapeResizeHack);
	resizer();
}

function netscapeResizeHack(){
	var ccc = MS_GetEl('contentArea');
	var hhh = 0;
	for(var iii=0; iii<ccc.childNodes.length; iii++){
		if(ccc.childNodes[iii].offsetHeight) hhh += ccc.childNodes[iii].offsetHeight;
	}
	ccc.style.height = hhh;
}
...
function resizer()
{
	...
	try
	{
		if(location.href.indexOf('newsgroups') < 0)
		{
			MS_GetEl('mainRow').style.height = 'auto';
		}
	}
	catch(e){}
	...
	var bodyHeight = Math.max(Math.max(oBodyHeight, oHeight), sBodyHeight, cBodyHeight);
 	var t = cBodyHeight - usedHeight - 1;
	if(mainContent && mainContent.offsetHeight < t)
	{
		mainContent.style.height = t;
...
MS_AddEvent(window,'load',resizeRightNav);
// if one or more of the web parts in the rightNav is expanded by the content, adjust other web parts to the same width.
// also stretch widthFix to accommodate the content.
function resizeRightNav()
{
	try
	{
		var rightNav = MS_GetEl('thinColumn');
		if(rightNav.children.length <= 0) return;
		var maxWidth = 0;
		for(i = 0; i < rightNav.children.length; i++)
			maxWidth = rightNav.children[i].offsetWidth > maxWidth ? rightNav.children[i].offsetWidth : maxWidth;
		for(i = 0; i < rightNav.children.length; i++)
			if(rightNav.children[i].offsetWidth != maxWidth)
				rightNav.children[i].style.width = maxWidth;
	}
	catch(e){}
	var stretched = maxWidth - 180; /* should fit the designed width of rightNav web parts */
	if(stretched <= 0) return;
	try
	{
		var fixW = MS_GetEl('widthFix');
		fixW.style.width = fixW.offsetWidth + stretched;
	}
	catch(e){}
}
...
function InitASrch(state)
{
	...
	elem = document.frmaSrch.spid;
	InitSelect(elem,"SPID","gss");
...
{
		var elem = MS_GetEl("productfilter");
		if(elem)
		{
			var producthtml = '<select name="spid" onchange="ProductChanged(this);">';
			if(extraOp && extraOp != '') producthtml+=extraOp;
			if(allprod && allprod != '') producthtml+=allprod;
		...
		var elem1 = MS_GetEl("nspd");
		var elem2 = MS_GetEl("nspd1");
		var noschead = MS_GetEl("noschead");
		if(elem1 && elem2)
		{
		 
		 elem1.style.display = 'none';
		 elem2.style.display = 'none';
		 noschead.style.display = 'none';
		} 
	
		var prd = document.frmaSrch.spid;
		if(prd && prd.options.length > 0)
		{
...
{
	var prdcol = MS_GetEl("pwtcol");
	var prdcol1 = MS_GetEl("pwtcol1");
...
function ProductChanged(elem)
{
	var value = elem.options[elem.selectedIndex].value;
	DisplayProductFilter(value);
	if(value == "more") document.location.href="/selectindex/default.aspx?target=search";
}
...
{
	StatsDotNet.eventCollectionId |= 64;
	var elem;	
	// save Query Value
	elem = document.frmsrch.query;
...
{
	if(state == 'False') StatsDotNet.eventCollectionId |= 64;
	else StatsDotNet.eventCollectionId |= 128;
...
function logRange()
{
	StatsDotNet.eventCollectionId |= 256;
}
...
function DoSubmit(frm , mode)
{
	...
	for(i=0; i<frm.srch.length; i++)
	{
		if(frm.srch[i].checked)
		{
			...
			{
				if(mode == 'a') StatsDotNet.eventCollectionId |= 128;
				else  StatsDotNet.eventCollectionId |= 64;
...
function SolIdForStatsNet(sid)
{
	if(!StatsDotNet.disabled){ 
		...
		StatsDotNet.eventCollectionId |= 262144;
	}
}
...
function ContactUsFix(querystrings)
{
	var eCurrentA = null;
	for(var i=0; i < document.links.length; i++)
	{
		eCurrentA = document.links[i];
		if(eCurrentA.href.toLowerCase().indexOf('/common/survey.aspx') > -1)
		{ 
			eCurrentA.href = eCurrentA.href + '&' + querystrings;
		}		
	} 
}
function RegionSave(pfx)	
{
	...
	var newLN = MS_GetEl('LN').value;
	if(prevLN != 'blank' && prevLN != '')
	{
		if(prevLN != newLN)
		{
			StatsDotNet.eventCollectionId |= 131072 //Math.pow(2,17);
		}
	}
	setcookieval(cookiekey, newLN);
	MS_GetEl('regionform').submit;
}
